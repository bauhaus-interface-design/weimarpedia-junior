...
*/
			...
			Aloha.bind( 'aloha-selection-changed', function ( event, rangeObject ) {
				...
				if ( !that.ignoreNextSelectionChangedEvent &&
					...
					if ( foundMarkup ) {
						...
						that.hrefField.setTargetObject( foundMarkup, 'href' ); // title
						...
						if ( jQuery( '#' + that.hrefField.extButton.id ).length == 0 ) {
							...
							that.hrefUpdateInt = setInterval( function () {
								if ( jQuery( '#' + that.hrefField.extButton.id ).length > 0 ) { // the object was finally created
									...
									that.hrefField.setTargetObject( foundMarkup, 'title' );
						...
						Aloha.Log.debug(this, 'aloha-link-selected' );
		...
		createButtons: function () {
			...
			this.hrefField.setTemplate( '<span><b>{name}</b><br/>url:{url}<br/>title:{title}</span>' );
...
//					that.hrefField.setTargetObject(foundMarkup, 'href');
		...
		insertLink: function ( extendToWord ) {
			...
			if ( this.hrefField.extButton.el ) {
				jQuery( this.hrefField.extButton.el.dom ).attr( 'value', 'Artikel-Titel oder URL eingeben' ).select();
			}
			this.hrefChange();
		},
		...
		hrefChange: function () {
			...
			if (this.hrefField.getItem()){
				var title = this.hrefField.getItem().name;
				$(this.hrefField.getTargetObject()).attr("title", title);
			}
