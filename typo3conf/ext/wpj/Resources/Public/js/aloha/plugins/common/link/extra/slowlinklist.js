...
function ( Aloha, jQuery ) {
	...
	new ( Aloha.AbstractRepository.extend( {
		...
		query: function ( p, callback ) {
			...
			var url = "http://192.168.2.108/Aptana/wpj_typo3/index.php?id=1&type=10&tx_wpj_pi1%5Baction%5D=suggestTag&tx_wpj_pi1%5Bcontroller%5D=article&term=";
			url += p.queryString;
			...
			$.ajax({
				...
				success: function(json){
					results = [
						{ name: 'WPJ', url: 'wpj', type: 'website' },
					];
					$.each(json, function(index, item){
						console.log(item);
						results.push({ name: item.label, url: item.value, type: 'website' });
					});
					console.log(results);
				},
			});
